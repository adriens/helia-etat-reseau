...
class TestMaintenanceValid:
    ...
    def test_null_coupure(self):
        data = {**VALID, "duree_coupure_min_minutes": None, "duree_coupure_max_minutes": None,
                "impact": "A_DETERMINER"}
...
class TestEnums:
    def test_all_services_defined(self):
        expected = {
            "TELEPHONIE_FIXE", "TELEPHONIE_MOBILE", "INTERNET_FIXE",
            "INTERNET_MOBILE", "RESEAU_CUIVRE",
            "FIBRE_OPTIQUE", "LIAISONS_CELERIS_ETHERNET",
        }
        assert {s.value for s in Service} >= expected

    def test_all_provinces_defined(self):
        assert {p.value for p in Province} == {
            "PROVINCE_SUD", "PROVINCE_NORD", "ILES_LOYAUTE"
        }

    def test_all_impacts_defined(self):
        assert {i.value for i in Impact} == {
            "COUPURE_20_30_MIN", "COUPURE_30_MIN", "A_DETERMINER"
        }
